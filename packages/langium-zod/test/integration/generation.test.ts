...
const fixturePath = (name: string): string =>
  join(process.cwd(), 'packages/langium-zod/test/fixtures', name);
...
const hierarchyAstTypes: AstTypesLike = {
  interfaces: [
    { name: 'Element', properties: [{ name: 'name', type: 'ID', optional: false }] },
    {
      name: 'Entity',
      superTypes: ['Element'],
      properties: [{ name: 'features', type: 'Feature', assignment: '+=', optional: false }]
    },
    {
      name: 'DataType',
      superTypes: ['Element'],
...
describe('generation integration', () => {
  it('reads simple fixture and generates schema source for primitive/array/optional mappings', () => {
    ...
    expect(source).toContain(
      'export const AstNodeSchema = z.discriminatedUnion("$type", [TagSchema, GreetingSchema]);'
    );
  ...
  it('generates discriminated unions for hierarchy types', () => {
    expect(readFileSync(fixturePath('hierarchy.langium'), 'utf8')).toContain(
    ...
    expect(source).toContain(
      'export const AbstractElementSchema = z.discriminatedUnion("$type", [EntitySchema, DataTypeSchema]);'
    );
    expect(source).toContain(
      'export const AstNodeSchema = z.discriminatedUnion("$type", [ElementSchema, FeatureSchema, EntitySchema, DataTypeSchema]);'
    );
  });
  ...
  it('generates conformance artifact with strip-aware internals', () => {
    ...
    writeFileSync(
      astPath,
      'export interface Data { $type: "Data"; name: string; superType?: string }\n',
      'utf8'
    );
    ...
    try {
      ...
      expect(conformanceSource).toContain(
        'type _Fwd_Data = z.infer<typeof DataSchema> extends _Surface<AST.Data> ? true : never;'
      );
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  ...
  it('emits cross-reference factories only when validation mode is enabled', () => {
    ...
    expect(enhanced).toContain(
      'export function createVariableRefSchema(refs: VariableRefSchemaRefs = {})'
    );
  });
  ...
  it('deduplicates multi-value unions preserving unique members', () => {
    ...
    expect(source).toContain(
      'z.union([z.literal(">="), z.literal("<="), z.literal(">"), z.literal("<")])'
    );
        ...
        interfaces: [
          {
            name: 'Item',
            comment: 'An item',
            properties: [{ name: 'name', type: 'ID', optional: false, comment: 'Item name' }]
          }
        ],
        unions: []
      }
