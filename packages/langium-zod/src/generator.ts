 ...
 */
function expressionToBuilder(
  expression: ZodTypeExpression,
  lazyNames: ReadonlySet<string> = new Set()
): BaseBuilder {
  switch (expression.kind) {
    ...
    case 'union':
      return build.union(
        expression.members.map((member) => expressionToBuilder(member, lazyNames))
      );
    case 'lazy':
...
function propertyReferencesAnyCycleMember(
...
function renderPropertyExpression(
  ...
  const withArrayMin =
    property.zodType.kind === 'array' && typeof property.minItems === 'number'
...
function renderCrossRefPropertyExpression(
  property: ZodPropertyDescriptor,
  formMetadata?: boolean
): string {
...
function propertyLine(
  ...
  const shouldUseGetter =
    recursiveTypes.has(ownerTypeName) &&
 ...
 */
function topoSortObjectDescriptors(
  descriptors: ZodObjectTypeDescriptor[],
  recursiveTypes: Set<string>
): ZodObjectTypeDescriptor[] {
...
export function generateZodCode(
  ...
  const hasCrossReferences = surfaceDescriptors.some(
    (descriptor) =>
      descriptor.kind === 'object' &&
      descriptor.properties.some((property) => containsCrossReference(property.zodType))
  );
  ...
  const unionNames = new Set(
    surfaceDescriptors.filter((d) => d.kind === 'union').map((d) => d.name)
  );
  ...
  for (const descriptor of surfaceDescriptors) {
    ...
    const zodExpr =
      descriptor.primitive === 'number'
        ? 'z.number()'
        : descriptor.primitive === 'boolean'
          ? 'z.boolean()'
          : descriptor.primitive === 'bigint'
            ? 'z.bigint()'
            : 'z.string()';
    lines.push(`export const ${descriptor.name}Schema = ${zodExpr};`);
    lines.push('');
  ...
  const objectDescriptors = surfaceDescriptors.filter(
    (d): d is ZodObjectTypeDescriptor => d.kind === 'object'
  );
  ...
  for (const descriptor of sortedObjects) {
    const hasRecursiveGetter =
      recursiveTypes.has(descriptor.name) &&
    ...
    for (const property of descriptor.properties) {
      objectProperties[property.name] = build.raw(
        renderPropertyExpression(property, unionNames, options.formMetadata)
      );
    }
  ...
  if (options.crossRefValidation) {
    for (const descriptor of sortedObjects) {
      const crossRefProperties = descriptor.properties.filter((property) =>
        containsCrossReference(property.zodType)
      );
