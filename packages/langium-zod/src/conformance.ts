...
export function generateConformanceSource(options: ConformanceGenerationOptions): {
  source: string;
  ...
  for (const typeName of matchedSchemaNames) {
    ...
    lines.push(
      `type _Fwd_${typeName} = z.infer<typeof ${typeName}Schema> extends ${surfaceType} ? true : never;`
    );
    lines.push(
      `type _Rev_${typeName} = ${surfaceType} extends z.infer<typeof ${typeName}Schema> ? true : never;`
    );
    lines.push('');
  }
